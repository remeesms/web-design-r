(function() {
	function g(j, l, o) {
		var p = j.control, m = j.opts.type = p._dragType, k = o.domTarget, n = jq(j.node), h = jq(
				'<div id="zk_ddghost"></div>').attr("style", n.attr("style"))
				.addClass("zswidget zswidget-drag z-drag-ghost").width(
						n.width() + "px").height(n.height() + "px").appendTo(
						p._sheet.sp.comp);
	...
	function f(G, A, u) {
		...
		if ("onWidgetSize" == j && z) {
			...
			if ("N" == z) {
				var C = zk(m.sp.comp).revisedOffset(), E = zk(D)
						.revisedOffset(), h = e(C, D, u.pageY), v = h[0], l = h[1];
				jq(B).height(l + "px");
				t.x1 = E[0] + s;
				...
				if ("EN" == z) {
					var C = zk(m.sp.comp).revisedOffset(), E = zk(D)
							.revisedOffset(), k = B.offsetLeft, h = e(C, D,
							u.pageY), v = h[0], l = h[1], r = c(zk(B)
							.revisedOffset(), D, u.pageX);
					...
					if ("E" == z) {
						var C = zk(B).revisedOffset(), E = zk(D)
								.revisedOffset(), k = B.offsetLeft, v = B.offsetTop, r = c(
								C, D, u.pageX);
						jq(B).width(r + "px");
						t.x1 = E[0] + s;
						t.y1 = E[1] + s;
						...
						if ("ES" == z) {
							...
							if ("S" == z) {
								var C = zk(B).revisedOffset(), E = zk(D)
										.revisedOffset(), k = B.offsetLeft, v = B.offsetTop, l = d(
										C, D, u.pageY);
								jq(B).height(l + "px");
								t.x1 = E[0] + s;
								t.y1 = E[1] + s;
								...
								if ("WS" == z) {
									var C = zk(m.sp.comp).revisedOffset(), E = zk(
											D).revisedOffset(), l = d(zk(B)
											.revisedOffset(), D, u.pageY), J = b(
											C, D, u.pageX), k = J[0], r = J[1];
									jq(B).width(r + "px").height(l + "px");
									t.x1 = k + C[0];
									t.y1 = E[1] + s;
									t.x2 = t.x1 + r - 4;
									...
									if ("W" == z) {
										var C = zk(m.sp.comp).revisedOffset(), E = zk(
												D).revisedOffset(), J = b(C, D,
												u.pageX), v = D.offsetTop, k = J[0], r = J[1];
										jq(B).width(r + "px");
										t.x1 = k + C[0];
										t.y1 = E[1] + s;
										t.x2 = t.x1 + r - 4;
										...
										if ("WN" == z) {
											var C = zk(m.sp.comp)
													.revisedOffset(), E = zk(D)
													.revisedOffset(), J = b(C,
													D, u.pageX), h = e(C, D,
													u.pageY), v = h[0], l = h[1], k = J[0], r = J[1];
											jq(B).width(r + "px").height(
													l + "px");
											t.x1 = k + C[0];
											t.y1 = v + C[1];
											t.x2 = t.x1 + r - 4;
			...
			var i = G.opts.innerOffset, m = G.control._sheet, F = m.$n("sp"), C = zk(
					m.sp.comp).revisedOffset(), I = m.leftWidth, H = m.topHeight, q = u.pageX
					- C[0] - i[0] + F.scrollLeft, o = u.pageY - C[1] - i[1]
					+ F.scrollTop;
	...
	function a(s, q) {
		var n = s.control, p = s.opts, k = s.control._sheet, x = k.sheetid, y = k.leftWidth, h = k.topHeight, r = zk(
				k.dp.comp).revisedOffset(), m = s.node.firstChild, j = k.custColWidth, l = k.custRowHeight, w = zss.SSheetCtrl
				._calCellPos(k, p.x1, p.y1, false), v = zss.SSheetCtrl
				._calCellPos(k, p.x2, p.y2, false), i = p.x1 - r[0] - y
				- j.getStartPixel(w[1]) + 1, o = p.y1 - r[1] - h
				- l.getStartPixel(w[0]) + 1;
		...
		n._sheet._wgt.fireMoveWidgetEvt(n.getType(), s.opts.type, s.control
				.getId(), Math.round(i), Math.round(o), Math.round(dx2), Math
				.round(dy2), w[1], w[0], v[1], v[0]);
			...
			.$extends(
					zul.Widget,
					{
						...
						adjustLocation : function(s) {
							...
							var j = this.getCol(), t = this.getRow(), q = p.custColWidth, u = p.custRowHeight, m = q
									.getSize(j), h = u.getSize(t), k = this._leftPos = p.leftWidth
									+ q.getStartPixel(j)
									+ (this.getLeft() > m ? m : this.getLeft()), r = this._topPos = p.topHeight
									+ u.getStartPixel(t)
									+ (this.getTop() > h ? h : this.getTop()), o = this._focus, l = this._focusBorderSize;
							jq(i).css( {
						...
						doMouseMove_ : function(A) {
							if (!this._dragging) {
								var r = A.domTarget, i = this.$n(), k = this._sheet.sheetid, s = k
										+ "-sheet", j = "#" + k;
								if ((r == i || r == this.$n("cave"))
										&& this.isSizable()) {
									this._dragType = "onWidgetSize";
									var t = zk(i).revisedOffset(), C = this._focusBorderSize, v = A.pageX
											- t[0], u = A.pageY - t[1], z = i.clientWidth, o = i.clientHeight, q = C, p = z
											- C, B = C, m = o - C;
									if (v <= q && u <= B) {
										this._dragDir = "WN";
										zcss.setRule(j + " .zswidget-focus",
												"cursor", "nw-resize", true, s)
									} else {
										if (v <= q && u <= m) {
											this._dragDir = "W";
											zcss.setRule(
													j + " .zswidget-focus",
													"cursor", "w-resize", true,
													s)
										} else {
											if (v <= q && u > m) {
												this._dragDir = "WS";
						...
						doMouseDown_ : function(h) {
							...
							if (!this._focus) {
								...
								var k = jq(this.$n()), i = this._focusBorderSize, n = this._leftPos
										- i, m = this._topPos - i;
						...
						doKeyDown_ : function(h) {
							var i = h.keyCode;
							this._sheet._wgt.fireWidgetCtrlKeyEvt(this._type,
									this._id, i, !!h.ctrlKey, !!h.shiftKey,
									!!h.altKey)
						},
