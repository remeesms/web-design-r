...
public final class GenericCertificate implements Serializable, XMLConstants {
	...
	public final synchronized void sign(Object paramObject, PrivateKey paramPrivateKey, Signature paramSignature)
		...
		try {
			byte[] arrayOfByte = PersistenceService.store2ByteArray(paramObject);
			paramSignature.initSign(paramPrivateKey);
			paramSignature.update(arrayOfByte);
			String str = new String(Base64Coder.encode(paramSignature.sign()));
			setEncoded(new String(arrayOfByte, "UTF-8"));
			setSignature(str);
	...
	public final synchronized void verify(PublicKey paramPublicKey, Signature paramSignature)
		...
		try {
			...
			byte[] arrayOfByte2 = Base64Coder.decode(getSignature());
	...
	public synchronized void setEncoded(String paramString) throws GenericCertificateIsLockedException {
		...
		if (paramString.equals(this.encoded))
			return;
	...
	public synchronized void setSignature(String paramString) throws GenericCertificateIsLockedException {
		...
		if (paramString.equals(this.signature))
			return;
		PropertyChangeEvent localPropertyChangeEvent = new PropertyChangeEvent(this, "signature", getSignature(),
				paramString);
		if (isLocked())
			throw new GenericCertificateIsLockedException(localPropertyChangeEvent);
	...
	public synchronized void setSignatureAlgorithm(String paramString) throws GenericCertificateIsLockedException {
		...
		if (paramString.equals(this.signatureAlgorithm))
			return;
	...
	public synchronized void setSignatureEncoding(String paramString) throws GenericCertificateIsLockedException {
		...
		if (paramString.equals(this.signatureEncoding))
			return;
