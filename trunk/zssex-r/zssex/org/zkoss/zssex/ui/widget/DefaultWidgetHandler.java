...
public class DefaultWidgetHandler implements WidgetHandler, Serializable {
	...
	public DefaultWidgetHandler() {
		this._clientWidgets = new ArrayList<WidgetCtrl>();
		this._nonClientWidgets = new ArrayList<WidgetCtrl>();
	}
	...
	public boolean addWidget(Widget widget) {
		...
		if (r) {
			...
			if (!(this._spreadsheet.isInvalidated())) {
				...
				onLoadWidgetOnDemand((BaseWidget) widget, sheet, rect.getLeft(), rect.getTop(), rect.getRight(),
						rect.getBottom());
			}
		}
	...
	public boolean redrawWidget(Widget widget) {
		...
		Worksheet sheet = this._spreadsheet.getSelectedSheet();
		Rect rect = ((SpreadsheetCtrl) this._spreadsheet.getExtraCtrl()).getVisibleRect();
		boolean r = onLoadWidgetOnDemand((BaseWidget) widget, sheet, rect.getLeft(), rect.getTop(), rect.getRight(),
				rect.getBottom());
		if (r)
