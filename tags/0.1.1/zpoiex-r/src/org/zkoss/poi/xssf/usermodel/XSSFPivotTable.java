  ...
  implements PivotTable
{
  ...
  private void initPivotFields() {
    ...
    for (CTPivotField f : pivotFields.getPivotFieldList()) {
      XSSFPivotField pf = new XSSFPivotField(f, (PivotCache.CacheField)iterator.next(), this);
  ...
  public void setRowItems(List<List<ItemInfo>> items)
  {
    ...
    for (int i = 0; i < itemSize; ++i) {
      ...
      ItemInfo info = getNonDataTypeItem(row);
      if (info != null) {
        int index;
